...
{
    //AES128
    if (Nk == 4 && K->size() != 16) return 1;
    //AES192
    else if (Nk == 6 && K->size() != 24) return 1;
    //AES256
    else if (Nk == 8 && K->size() != 32) return 1;
...
void AES::MixColumns(Array* state)
{
    ...
    {
        temp_state->data[i + 0 * Nb] =
            multiply_gf(0x02, state->data[i + 0 * Nb]) ^
        ...
        temp_state->data[i + 3 * Nb] =
            multiply_gf(0x03, state->data[i + 0 * Nb]) ^
            state->data[i + 1 * Nb] ^
            state->data[i + 2 * Nb] ^
